...
The final report is `prototype_report_v2.qmd` (Quarto → PDF). See Section 4 ("Results and Methods") for the main analysis and Appendices A–D for extended detail.
...
├── prototype_report_v2.qmd    # Final report source (Quarto)
├── prototype_report_v2.pdf    # Rendered report
├── requirements.txt
│
├── data/
...
| LSEG   | USDOIS2Y=PYNY, USDOIS5Y=PYNY, USDOIS10Y=PYNY | EFFR OIS par rates | Swap rates |
| LSEG   | .MOVE           | ICE BofA MOVE Index | Implied Treasury vol |

Final panel: 1,890 daily observations spanning 2019-01-01 → 2026-04-14. LSEG tickers reference the **EFFR** leg, not pure SOFR; we note this as a minor caveat in the report.
...
```bash
quarto render prototype_report_v2.qmd
```

This produces `prototype_report_v2.pdf`. The Quarto file uses a LaTeX PDF engine (default `xelatex` or `pdflatex`). Running raw `pandoc` on the `.qmd` file is **not** supported — Quarto-specific YAML options (e.g., `linestretch`) and short-code cross-references require the Quarto toolchain.
